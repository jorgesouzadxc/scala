...
object Main extends App {
    ...
    println("Filmes ordenados por rating:")
    val films = directors.flatMap(_.films)
    println(s"\nFilms ${films.toString}\n")
    films
        .sortWith((a, b) => a.imdbRating < b.imdbRating)
        .foreach((film) => {
            println(s"Filme ${film.name}, rating ${film.imdbRating}")
        })
    
    // lazy val averageRating = films.foldLeft(0.0)((a, b) => {
    //     println(a.imdbRating)
    ...
    directors.foreach((director) => {
        director.getFilmsNames.foreach((filmName) => {
            println(s"Tonight only! ${filmName} by ${director.getFullName}!")
        })
    })
}